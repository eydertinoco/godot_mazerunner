...
func initialize_maze():
	...
	for x in range(map_size.x):
		...
		for y in range(map_size.y):
			row.append(0) # Todas as células começam como paredes
...
func connect_start_to_exit():
	...
	var exit_cell = Vector2(exit_point.x - 1, exit_point.y - 1) # Ajustando para as coordenadas do labirinto gerado
